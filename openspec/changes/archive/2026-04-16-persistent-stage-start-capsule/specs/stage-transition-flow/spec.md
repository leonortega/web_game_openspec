...
- **AND** a short synthesized intro stinger for that specific stage plays when audio is available and hands off cleanly once the in-world arrival and door-close beat resolve and gameplay begins
...
- **AND** that handoff may include a short post-reveal start-capsule door-close beat while remaining automatic and bounded
...
- **AND** the existing stage flow order and timing semantics remain unchanged apart from an optional bounded stage-start arrival and door-close beat after the intro surface
