...
The game SHALL present core gameplay status such as stage identity, coin count, health, and active powers in a single horizontal top-aligned HUD band during active play. Secondary stage metadata such as run label and segment name SHALL appear as tiny bottom-right text instead of inside the primary HUD band.
