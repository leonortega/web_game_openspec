...
#### Scenario: Rejecting tolerance-masked grounded enemy catalog drift
- **WHEN** focused automated validation checks shipped grounded non-flying enemy entries in authored stage catalog
- **THEN** each entry already sits on readable intended support in source data
- **AND** validation reports entries that only appear grounded after tolerant normalization or similar runtime correction