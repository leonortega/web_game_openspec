...
- **AND** a short synthesized intro stinger for that specific stage plays when audio is available and hands off cleanly once the in-world arrival, scripted walk-out, and capsule close beat resolve and gameplay begins
...
- **AND** that automatic handoff now includes the scripted walk-out and capsule close sequence without requiring extra player input or hiding current stage-status context
...
- **AND** the existing stage flow order and timing semantics remain unchanged apart from an optional bounded stage-start arrival, scripted walk-out, and capsule close beat after the intro surface
