...
- **AND** that presentation hands off into a short capsule-arrival appearance beat that reverses the exit-style finish language before player control starts
...
- **AND** the new stage uses the same bounded capsule-arrival appearance beat and scripted walk-out before active control starts
...
- **AND** the checkpoint respawn does not trigger the stage-start capsule-arrival sequence or scripted walk-out

#### Scenario: Walking out from the start capsule
- **WHEN** the stage-start arrival finishes materializing the player at the authored spawn
- **THEN** the player automatically performs a short deterministic walk-out from the capsule while remaining non-interactive
- **AND** normal active play does not begin until that walk-out and the following capsule close beat resolve

#### Scenario: Closing the persistent start capsule
- **WHEN** the scripted start-capsule walk-out completes on a fresh stage start
- **THEN** the remaining start capsule plays a short bounded door-close animation before active control begins
